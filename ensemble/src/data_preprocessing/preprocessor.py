...
class TrajectoryPreprocessor:
    ...
    def _merge_point_track_data(self, point_data: pd.DataFrame, track_data: pd.DataFrame) -> Optional[pd.DataFrame]:
        """
        合并点迹和航迹数据
        ...
        try:
            # 按时间和批号合并
            merged = pd.merge(track_data, point_data, 
                            on=['时间', '批号'], 
                            how='inner')
            
            if len(merged) == 0:
                # 如果完全匹配失败，尝试按批号合并并选择最接近的时间
                if self.verbose:
                    print("精确时间匹配失败，尝试最近时间匹配")
                
                # 按批号分组合并
                merged_list = []
                for batch_id in track_data['批号'].unique():
                    track_batch = track_data[track_data['批号'] == batch_id]
                    point_batch = point_data[point_data['批号'] == batch_id]
                    
                    if len(track_batch) == 0 or len(point_batch) == 0:
                        continue
                    
                    # 对于每个航迹点，找到最接近的点迹点
                    for _, track_row in track_batch.iterrows():
                        time_diffs = np.abs((point_batch['时间'] - track_row['时间']).dt.total_seconds())
                        closest_idx = time_diffs.idxmin()
                        
                        # 如果时间差小于阈值（比如1秒），则合并
                        if time_diffs[closest_idx] < 1.0:
                            point_row = point_batch.loc[closest_idx]
                            merged_row = pd.concat([track_row, point_row.drop(['时间', '批号'])])
