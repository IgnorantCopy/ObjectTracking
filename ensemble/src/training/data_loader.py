...
class TrajectoryDataLoader:
    ...
    def __init__(self, 
        ...
        if pretrained_scaler is not None:
            self._load_scaler(pretrained_scaler)
        else:
            self.scaler = None
    ...
    def _preprocess_data():
        ...
        return preprocessor.process_all_trajectories()
    ...
    def _load_and_prepare_data(self):
        ...
        if self.normalize:
            ...
            # 标准化所有数据集
            train_sequences = self._normalize_sequences(train_sequences)
            val_sequences = self._normalize_sequences(val_sequences)
            test_sequences = self._normalize_sequences(test_sequences)
    ...
    def inverse_transform(self, sequences: np.ndarray) -> np.ndarray:
        ...
        original_shape = sequences.shape
        sequences_flat = sequences.reshape(-1, sequences.shape[-1])
        sequences_original = self.scaler.inverse_transform(sequences_flat)
        return sequences_original.reshape(original_shape)
