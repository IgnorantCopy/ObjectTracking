...
def cfar_detector_2d(rd_matrix_db,
    ...
    """
    对RD矩阵执行2D-CFAR检测 - 调整合理的阈值
    """
    
    print(f"CFAR检测开始:")
    print(f"  输入矩阵形状: {rd_matrix_db.shape}")
    print(f"  数据范围: [{np.min(rd_matrix_db):.1f}, {np.max(rd_matrix_db):.1f}] dB")
    ...
    # 2. 杂波分析
    zero_vel_idx = rd_matrix_db.shape[1] // 2
    clutter_width = 6  # 减小杂波区域范围
    clutter_region = rd_matrix_db[:, max(0, zero_vel_idx-clutter_width):min(rd_matrix_db.shape[1], zero_vel_idx+clutter_width+1)]
    clutter_power = np.mean(clutter_region[clutter_region > -np.inf])
    clutter_max = np.max(clutter_region[clutter_region > -np.inf])
    
    # 分析非杂波区域
    non_clutter_mask = np.ones(rd_matrix_db.shape[1], dtype=bool)
    non_clutter_mask[max(0, zero_vel_idx-clutter_width):min(rd_matrix_db.shape[1], zero_vel_idx+clutter_width+1)] = False
    non_clutter_region = rd_matrix_db[:, non_clutter_mask]
    target_power = np.max(non_clutter_region[non_clutter_region > -np.inf]) if non_clutter_region.size > 0 else -np.inf
    
    scr_db = target_power - clutter_power
    
    print(f"  杂波平均功率: {clutter_power:.1f} dB")
    print(f"  杂波最大功率: {clutter_max:.1f} dB") 
    print(f"  目标最大功率: {target_power:.1f} dB")
    print(f"  信杂比: {scr_db:.1f} dB")
    
    # 3. 调整后的质量检查
    
    # 检查1: 基本信杂比要求 - 降低阈值
    min_scr_threshold = 6.0  # 降低到6dB
    if scr_db < min_scr_threshold:
        print(f"  -> 返回None: 信杂比过低 ({scr_db:.1f} < {min_scr_threshold})")
        return None
    
    # 检查2: 目标功率绝对阈值 - 降低阈值
    min_target_power = -25.0  # 降低到-25dB
    if target_power < min_target_power:
        print(f"  -> 返回None: 目标功率过低 ({target_power:.1f} < {min_target_power})")
        return None
    
    # 检查3: 移除杂波绝对功率限制，改为相对检查
    # 不再限制杂波的绝对功率值，因为这取决于系统设置
    
    # 检查4: 目标与杂波最大值的比较 - 这是关键检查
    target_vs_clutter_max = target_power - clutter_max
    min_target_clutter_diff = 3.0  # 降低到3dB
    if target_vs_clutter_max < min_target_clutter_diff:
        print(f"  -> 返回None: 目标未明显超过杂波峰值 ({target_vs_clutter_max:.1f} < {min_target_clutter_diff})")
        return None

    # 检查5: 杂波分布检查 - 新增更智能的杂波分析
    # 检查杂波是否异常分散（可能表示系统不稳定）
    clutter_std = np.std(clutter_region[clutter_region > -np.inf])
    max_clutter_std = 20  # 杂波标准差不应过大
    if clutter_std > max_clutter_std:
        print(f"  -> 返回None: 杂波分布过于分散 (std={clutter_std:.1f} > {max_clutter_std})")
        return None

    # 4. 转换为线性域
    rd_matrix_linear = 10 ** (rd_matrix_db / 20.0)

    # 5. 找到非杂波区域的最强点
    peak_pos = np.unravel_index(np.argmax(non_clutter_region), non_clutter_region.shape)
    non_clutter_cols = np.where(non_clutter_mask)[0]
    peak_row = peak_pos[0]
    peak_col = non_clutter_cols[peak_pos[1]]
    
    rows, cols = rd_matrix_linear.shape
    
    print(f"  最强点位置: ({peak_row}, {peak_col})")
    print(f"  最强点功率: {rd_matrix_db[peak_row, peak_col]:.1f} dB")
    
    # 检查6: 目标点质量检查 - 降低标准
    target_region_size = 2  # 减小检查区域
    target_region = rd_matrix_db[
        max(0, peak_row-target_region_size):min(rows, peak_row+target_region_size+1),
        max(0, peak_col-target_region_size):min(cols, peak_col+target_region_size+1)
    ]
    target_region_std = np.std(target_region[target_region > -np.inf])
    
    max_target_std = 12.0  # 放宽到12dB
    if target_region_std > max_target_std:
        print(f"  -> 返回None: 目标区域信号杂乱 (std={target_region_std:.1f} > {max_target_std})")
        return None

    # 检查7: 目标孤立性检查 - 新增
    # 确保目标是一个相对孤立的峰值，而不是连续高值区域的一部分
    isolation_region_size = 5
    isolation_region = rd_matrix_db[
        max(0, peak_row-isolation_region_size):min(rows, peak_row+isolation_region_size+1),
        max(0, peak_col-isolation_region_size):min(cols, peak_col+isolation_region_size+1)
    ]
    # 计算目标点在其邻域中的突出程度
    isolation_region_no_peak = np.copy(isolation_region)
    center_r = min(isolation_region_size, peak_row)
    center_c = min(isolation_region_size, peak_col - max(0, peak_col-isolation_region_size))
    if center_r < isolation_region.shape[0] and center_c < isolation_region.shape[1]:
        isolation_region_no_peak[center_r, center_c] = -np.inf
    
    second_max = np.max(isolation_region_no_peak[isolation_region_no_peak > -np.inf])
    target_prominence = target_power - second_max
    
    min_prominence = 1.5  # 目标必须比周围次高点高1.5dB以上
    if target_prominence < min_prominence:
        print(f"  -> 返回None: 目标不够突出 (prominence={target_prominence:.1f} < {min_prominence})")
        return None

    # 6. 定义检测区域
    local_radius = 3
    detection_area_rows = np.arange(
        max(0, peak_row - local_radius),
        min(rows, peak_row + local_radius + 1)
    )
    detection_area_cols = np.arange(
        max(0, peak_col - local_radius),
        min(cols, peak_col + local_radius + 1)
    )
    
    print(f"  检测区域: 行[{detection_area_rows[0]}, {detection_area_rows[-1]}], 列[{detection_area_cols[0]}, {detection_area_cols[-1]}]")
    ...
    target_detected, target_row, target_col = func_ca_cfar_detect(
        ...
        10 ** (cfar_threshold_factor_db / 10)
    )

    print(f"  CFAR结果: {target_detected}")
    if target_detected:
    ...
    if target_detected:
        # 确保检测到的目标不在杂波区域
        if abs(target_col - zero_vel_idx) <= clutter_width:
            print(f"  -> 返回None: 检测到的目标在杂波区域内")
            return None
...
def func_ca_cfar_detect(data_amplitude, detection_rows, detection_cols, Gr, Gd, Td, threshold_factor):
    ...
    detection_rows = detection_rows[
        (detection_rows >= 0) & (detection_rows < num_rows)
        ]
    detection_cols = detection_cols[
        (detection_cols >= 0) & (detection_cols < num_cols)
        ]
    ...
    flat_indices = np.argsort(detection_area_data.ravel())[::-1]
    ...
    for k in range(len(flat_indices)):
        ...
        if left_noise_count > 0 and right_noise_count > 0:
            ...
            # 放宽稳定性检查
            avg_ratio = max(left_avg, right_avg) / (min(left_avg, right_avg) + 1e-10)
            if avg_ratio > 4.0:  # 放宽到4倍
                continue
        ...
        # 降低参考单元数要求
        min_ref_cells = 8  # 降回8个参考单元
        if total_noise_count < min_ref_cells:
            continue
        ...
        with open(batch.raw_file, 'rb') as fid:
            while True:
                ...
                try:
                    ...
                    print(f"帧 {frame_count}: MTD处理完成，开始CFAR检测")
                    ...
                    processed_rd = cfar_detector_2d(
                        ...
                        cfar_threshold_factor_db=6.5
                    )

                    print(f"帧 {frame_count}: CFAR检测完成，结果: {processed_rd is not None}")
 
                    if processed_rd is None:
                        print(f"帧 {frame_count}: 未找到目标，跳过此帧")
                        continue  # 未找到目标，跳过此帧
 
                    print(f"帧 {frame_count}: 找到目标，添加到结果列表")
                    rd_matrix = processed_rd

                    rd_matrices.append(rd_matrix)
                    ranges.append(range_axis)
                    velocities.append(velocity_axis)
                    print(f"帧 {frame_count}: 成功添加到结果列表，当前总数: {len(rd_matrices)}")

                except Exception as e:
                    # 静默跳过有问题的帧，避免过多错误输出
                    print(f"帧 {frame_count}: 处理异常 - {str(e)}")
                    continue
